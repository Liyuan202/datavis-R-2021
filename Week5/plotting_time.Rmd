...
# this works:
tidytime %>%
  mutate(activity_class = recode(activity_class, t_a01 = "lifts") %>% factor)
...
tidytime %>%
  ...
  count(minutes) %>%
  filter(minutes > 0) %>%
  mutate(highlight_count = if_else(minutes %in% highlight_times, n, NA_integer_)) %>% # if highlighted, value = count
